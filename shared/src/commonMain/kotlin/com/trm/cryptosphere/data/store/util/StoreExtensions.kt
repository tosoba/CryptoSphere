package com.trm.cryptosphere.data.store.util
...
fun <Key : Any, Local : Any, Output : Any> SourceOfTruth<Key, Local, Output>.usingDispatchers(
  ...
  return object : SourceOfTruth<Key, Local, Output> {
    ...
    override suspend fun write(key: Key, value: Local) =
      withContext(writeDispatcher) { wrapped.write(key, value) }

    override suspend fun delete(key: Key) = withContext(writeDispatcher) { wrapped.delete(key) }

    override suspend fun deleteAll() = withContext(writeDispatcher) { wrapped.deleteAll() }
  }
}
