...
interface TokenDao {
  ...
  @Query(
    ...
    SELECT 
        T2.*, 
        GROUP_CONCAT(DISTINCT TT3.tag_name) AS all_tag_names, 
        GROUP_CONCAT(DISTINCT TT2.tag_name) AS shared_tag_names 
    FROM token AS T1
    ...
    GROUP BY T2.id
    ORDER BY CASE WHEN T2.id = :id THEN 0 ELSE 1 END, COUNT(DISTINCT TT2.tag_name) DESC, T2.cmc_rank
    """
  )
