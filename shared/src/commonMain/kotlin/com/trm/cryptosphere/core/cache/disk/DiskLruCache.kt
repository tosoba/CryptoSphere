...
import com.trm.cryptosphere.core.cache.util.FaultHidingSink
import com.trm.cryptosphere.core.cache.util.createFile
import com.trm.cryptosphere.core.cache.util.deleteContents
...
internal class DiskLruCache(
  ...
  private fun newJournalWriter(): BufferedSink {
    ...
    val faultHidingSink = FaultHidingSink(fileSink) { hasJournalErrors = true }
  ...
  /**
   * Computes the current size and collects garbage as a part of initializing the cache. Dirty
   * entries are assumed to be inconsistent and will be deleted.
   */
  ...
  inner class Editor(val entry: Entry) : SynchronizedObject() {
    ...
    /**
     * Get the file to read from/write to for [index]. This file will become the new value for this
     * index if committed.
     */
    private fun file(index: Int): Path =
      synchronized(this) {
